...
from dynamic.login_helpers import (
    fill_username_and_trigger_role,
    find_role_select,
    perform_language_selection,
    perform_role_selection,
)
from dynamic.common_actions import infer_action_from_button
from dynamic.module_filter import matches_module, parse_module_from_description, slugify_module
...
def _return_to_dashboard(driver: WebDriver, wait: WebDriverWait, post_login_url: str) -> None:
    try:
        from automation.selenium_session import navigate
...
def _explore_module_page(
    ...
    from dynamic.page_intelligence import analyze_current_page

    steps: list[StepDef] = []
    step_no = 1
    nav_xpath = _module_nav_xpath(link_text)
    if l_by == "xpath" and "//nav" not in l_val:
        l_by, l_val = "xpath", nav_xpath
    ...
    try:
        ...
        if not _click_by_locator(driver, l_by, l_val):
...
def discover_application(
    ...
    if automation_only:
        ...
        for i, (link_text, el) in enumerate(nav_links):
            l_key, l_by, l_val = _best_locator(el)
            nav_xpath = _module_nav_xpath(link_text)
            locators[f"nav_{i+1}"] = {"by": "xpath", "value": nav_xpath}
            modules_catalog.append(
                {
                    "id": slugify_module(link_text),
                    "name": link_text,
                    "locator": {"by": "xpath", "value": nav_xpath},
                    "scenario_ids": [],
                }
            )
        ...
        from dynamic.page_intelligence import (
            save_page_map,
