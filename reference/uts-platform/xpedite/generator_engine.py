...
def _business_flow_bc_name(component="", description="", scenario_name="", fallback="Business_Flow"):
    ...
    try:
        from dynamic.module_filter import business_flow_bc_name as _bf
...
def process_offline_generation(file_path):
    ...
    try:
        ...
        _inp = _P(__file__).resolve().parent.parent / "config" / "app-input.json"
        if _inp.exists():
            _desc = (json.loads(_inp.read_text(encoding="utf-8-sig")).get("description") or "").strip()
            if _desc:
                try:
                    from dynamic.module_filter import matches_module, parse_module_from_description
